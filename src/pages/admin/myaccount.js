...
const Myaccount = () => {
  ...
  const[user,setUser]=useState({value:null});
     ...
     const handleSaveChange=async()=>{
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/updateadmin`, {
        method: "POST", // or 'PUT'
        headers: {
          "Content-Type": "application/json",
        },
     ...
     const handleSaveChangePassword=async()=>{
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/updateadminpassword`, {
        method: "POST", // or 'PUT'
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <>
    <Head>
      <title>Admin MyAccount| Manage Reservations and Culinary Services</title>
...
/>
      ...
      <h2 className="font-bold md:text-xl text-lg mx-2">1.Admin Details</h2>
        ...
        <div className="px-2 w-1/2">
          <div className="mb-4">
            ...
            value={user.email}
              ...
              id="email"
              name="email"
              className="w-full bg-white rounded border border-gray-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" readOnly={true}
            />:<input
      ...
      <button onClick={handleSaveChange}
          className="m-2 mb-5 disabled:bg-pink-300 flex mr-2 text-white bg-amber-500 border-0 py-2 px-2 focus:outline-none hover:bg-amber-600 rounded text-sm"
        >
          Save Changes
        </button>
      ...
      <button onClick={handleSaveChangePassword}
          className="m-2 mb-5 disabled:bg-pink-300 flex mr-2 text-white bg-amber-500 border-0 py-2 px-2 focus:outline-none hover:bg-pink-600 rounded text-sm"
        >
          Save Changes
        </button>
    </div>}
