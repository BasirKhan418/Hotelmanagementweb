   ...
   <Swiper
    ...
    <div className="px-5 pb-5">
        ...
        {/* <span className="text-3xl font-bold text-gray-400 dark:text-white line-through ">$599</span> */}
            <span className="sm:text-3xl text-xl font-bold text-gray-900 dark:text-white">₹{item.price}/starting</span>
...
export async function getServerSideProps(context){
    if(!mongoose.connections[0].readyState){
      ...
      let catroom= await Categoryfood.find();
    return{
      props:{catroom:JSON.parse(JSON.stringify(catroom))}
    }
  }
export default Food
