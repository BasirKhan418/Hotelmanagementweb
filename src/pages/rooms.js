...
const Rooms = ({deluxe,exeucative,family,other}) => {
  ...
  <div className="mt-4 px-5 pb-5">
    ...
    <div className='text-black dark:text-white h-16 flex bg-white items-center'>
      <h1 className='sm:text-2xl text-xl font-bold mx-8'>Other Rooms</h1>
...
let deluxe = await Product.find({category:"room",subcategory:"deluxe"});
let exeucative = await Product.find({category:"room",subcategory:"exeucative"})
let family = await Product.find({category:"room",subcategory:"family"})
let other = await Product.find({category:"room",subcategory:{$nin:["exeucative","deluxe","family"]}})
  return {
      props: {deluxe:JSON.parse(JSON.stringify(deluxe)),exeucative:JSON.parse(JSON.stringify(exeucative)),family:JSON.parse(JSON.stringify(family)),other:JSON.parse(JSON.stringify(other))},
  };
}
